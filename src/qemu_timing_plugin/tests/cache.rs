...
fn test_fifo_eviction_scenario() {
    ...
    assert!(!hit_b, "Block 0x2000 should have been evicted by the re-access of 0x1000");
}
...
fn test_temporal_safety_with_no_domain() {
    ...
    assert!(violation.is_none(), "No violation should occur if previous access had no domain");
}
