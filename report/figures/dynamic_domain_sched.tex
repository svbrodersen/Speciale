\begin{tikzpicture}[font=\ttfamily]
    ...
    % Top most domain
    \path[draw, decorate, decoration=brace] (0, \vtopprio + 1) -- (\timeslice * 8, \vtopprio + 1) node[lbl, midway, above, yshift=0.3em]{Domain 1};
    \draw (0, \vtopprio-\barheight/2) rectangle (\timeslice*4, \vtopprio+\barheight/2) 
      node[lbl, midway]{Round robin};

    % Second domain
    \path[draw, decorate, decoration=brace] (\timeslice * 4, \vtopprio + 0.5) -- (\timeslice * 8, \vtopprio + 0.5) node[lbl, midway, above, yshift=0.3em]{Subdomain 1};
    ...
    \foreach \x in {0, ..., 4} {
         \draw (\x*\timeslice*2, \ytimeline-0.05) -- (\x*\timeslice*2, \ytimeline+0.05);
    }
