\section{Evaluation}
\fbox{I would want one more workload with multiple domains with both inter domain communication and cross-domain communication. This would be shorter, as the current demo would still be used to describe the setup. Should reference items that should be covered in the Introduction.}
...
each of which with a single privileged task. Once domain 0 has created these
...
ticks are constant for each domains. Running this demo with the '-icount' QEMU
...
other domains, before all the unprivileged tasks are the only one producing
