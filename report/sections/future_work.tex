...
\subsection{Verification on cycle accurate hardware}
This project has only been tested on the QEMU virtual system. This is clearly
not sufficient to empirically dispute the presence of timing channels as
mentioned in Section~\ref{sec:qemu_cycle_accuracy}. To do this a platform,
which supports the temporal fence instruction, would be needed. During the
evaluation phase of this thesis a QEMU TCG plugin was developed in an attempt
to bridge this gap, and provide a cache emulation of the L1 and L2 cache with
support of flushing the L1 cache on domain switches. While the plugin did
seemingly function correctly in the emulation, the plugin was limited in what
could be done. QEMU was never designed to produce cycle accurate emulation, and
as such when a plugin attempts to update the machine timer externally it seemed
this happened asynchronously and was affected by the host machine running QEMU.
This meant that even if the cache emulation was correct, there would be a non
deterministic execution from the guest OS point of view making the time control
unusable. While significant work went into the plugin, further investigation
into the issue found that there do exist cycle accurate emulators such as
gem5\cite{lowe-powerGem5SimulatorVersion2020}, and while other research have
found some pain points \cite{bossuet2023}, it could still prove interesting to
run the implementation on such a platform in the future if real hardware is
unavoidable. Such a simulator could significantly reduce the cost of entry and
lead to more rapid development in the pursuit of time protection.
...
domains time slice, and would only be able to convey information between the
...
The preemptable flag is used to flag whether or not the current lock
acquisition will respect a preemption. The preempt function call checks if mip
register has a bit set at (1 << 7), which corresponds to the Machine Timer
Interrupt Pending (MTIP) bit \cite{MipRISCVISA}. This bit is what indicates that
the machine timer is pending and should fire. In S3K this lock is grabbed in
two important places, namely:
...
That is, when we attempt to do a system call we acquire the ttas lock, and when
we attempt to schedule a new task we also grab the same lock. Now while the
preemptable flag is true for the syscall, it simply checks that no preemption
is pending while gathering the lock, once it has the lock, then it will delay
scheduling if the scheduler runs while the syscall holds the lock. The syscalls
in S3K attempt to rectify this by checking for preemption at after a constant
amount of work as to not block the scheduler for too long, but if this constant
time varies one could assume that these syscalls could be used nefariously and
a domain could in theory cause predictable scheduling delay and relay
information to the following domain. Now this analysis is purely theoretical,
and would require access to hardware to fully test and as such should mainly be
seen as some of the multitude of challenges a truly time protected kernel will
face. 
