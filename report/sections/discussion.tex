...
%TODO: Maybe gather this together with the Future Work section, and then expand
%on the introduction to the sections. If decide not to still provide a little
...
%TODO: Change this to a complexity added and how this could be approached.
%Don't repeat in regards to what happens in the conclusion. This feels more
%like a conclusion section at this point.
...
task's own domain is running. This breaks a lot of assumptions one might have
...
So while time protection seems to be a reachable goal, it comes with
significant drawbacks. It adds to the complexity of the kernel, hinders the
average performance and restricts how the kernel operates significantly. As
such, it seems unreasonable to aim for time protection across all future
kernels, and a general operating system should most likely not strive towards
full time protection. However, some aspects of the work done could translate.
As an example, one might want to create a single task for handling encryption,
where the microarchitectural state is cleared before and after its execution.
...
verifications are also necessary. At the time of writing there is no formally
verified kernel, which guarantees time protection, which these environments
would require. 
