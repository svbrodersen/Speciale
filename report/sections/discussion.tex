...
microarchitectural state on domain switch, a lot of the hardware still remains
...
where higher level caches more often makes use of elements of the physical
address.
...
In more recent years different architectures have added support for hardware
...
and something where depending on the usage case the correct solution could
vary.
...
until it can be scheduled again. This works via the global \textt{xTicks},
...
tasks own domain is running. This is breaks a lot of assumptions one might have
...
the micro architectural state is cleared before scheduling the next task.
...
verifications are also necessary. As of writing there is no formally verified
