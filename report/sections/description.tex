...
and as such it is not something that can be left untouched \cite{kocher2019,
lipp2018}.
...
\cite{cauligi2019}. This ensures that in an isolated
...
history independent context switch latency \cite{wistoff2023, wistoff2025}. Other work has
...
against with little impedance on performance \cite{ge2019}.
...
design and what hardware support is required?}
...
such as the \cite{karlsson2025}, to show that it is effective in showing cache based timing
channels. Further, this will then be used as the development basis to implement
timing protection in a non-timing safe kernel. Specifically this will be done
by looking into deterministic context switching, targeted flushing of the
microarchitectural state and constant-time system calls. These implementations
will then be evaluated in regards to the possible cache timing channels using
the aforementioned plugin and showcase whether it is feasible to implement
timing protection in a more standard kernel.
...
\begin{itemize}
  ...
  \item Design and implement a cache emulation layer capable of finding timing
    channels in the cache layer.
  \item Evaluate the efficacy and viability of the implementation by running
    already established microkernels.
